...
// Утечки, найденные широким замером полноты на 4549 свежих договорах (02.08).
describe("privacy/deid/precision: утечки широкого замера 02.08", () => {
  const per = (raw: string, index: number): DetectedEntity => ({
    type: "PER", raw, index, confidence: "high", source: "rule",
  });
  const one = (text: string, raw: string): DetectedEntity[] =>
  ...
  it("крымский телефон не считается ISBN и не снимает защиту с человека", () => {
    const text = "Уполномоченное лицо Заказчика: - Ромашкин Юрий Борисович тел. +7978-625-08-06, электронный адрес: a@b.ru";
    expect(one(text, "Ромашкин Юрий Борисович")).toHaveLength(1);
  });

  it("настоящий ISBN по-прежнему открывает автора", () => {
    const text = "Литературное чтение, ISBN 978-5-09-087674-7, Ромашкина Л.Ф., Российская 2026, 2 шт";
    expect(one(text, "Ромашкина Л.Ф.")).toEqual([]);
  });

  it("человек с датой рождения не мемориальное имя, даже если рядом «имени»", () => {
    const text = "|Электронная почта: a@b.ru |имени Ромашкин Игорь Владимирович, 2 апреля 1978 года рождения, место";
    expect(one(text, "Ромашкин Игорь Владимирович")).toHaveLength(1);
  });

  it("а школа имени человека по-прежнему не скрывается", () => {
    const text = "школа № 91 имени Ромашкина Михаила Александровича, именуемое в дальнейшем Заказчик";
    expect(one(text, "Ромашкина Михаила Александровича")).toEqual([]);
  });
});
