...
// Три ошибки из разметки владельца 02.08 — все адресные, механики разные.
describe("privacy/deid/precision: адресные дефекты разметки владельца 02.08", () => {
  const per = (raw: string, index: number): DetectedEntity => ({
    type: "PER", raw, index, confidence: "high", source: "rule",
  });
  const one = (text: string, raw: string): DetectedEntity[] =>
  ...
  it("литера здания — часть адреса", () => {
    const text = "Место нахождения: 191167, г.Санкт-Петербург, Синопская набережная, дом 14, Литера А.\nИНН 7707049388";
    expect(one(text, "Литера А.")).toEqual([]);
  });

  it("микрорайон в чью-то честь не скрывается, даже когда «им» оторвано вёрсткой", () => {
    const text = "410019, САРАТОВСКАЯ ОБЛАСТЬ, Г САРАТОВ, ЛН. 13-Я (2-Й ИМ |\n|Место нахождения: 410064 | |ПУГАЧЕВА Е.И. МКР.), Д. Е115";
    expect(one(text, "ПУГАЧЕВА Е.И.")).toEqual([]);
  });

  it("но живой подписант рядом с адресом остаётся скрытым", () => {
    // Значение НЕ заглавными — это и отличает подпись от выгрузки реестра.
    const text = "ГБУЗ Больница имени Пирогова, в лице главного врача Ромашкин И.И. ул. Ленина, д. 5";
    expect(one(text, "Ромашкин И.И.")).toHaveLength(1);
  });

  it("прилагательное от названия города — не фамилия", () => {
    const text = "Муниципальное бюджетное учреждение Ромашковского городского округа «Зеленстрой», именуемое";
    expect(one(text, "Ромашковского")).toEqual([]);
  });

  it("а фамилия на «-ского» у подписанта остаётся скрытой", () => {
    const text = "в лице директора Ромашковского, действующего на основании устава городского округа";
    expect(one(text, "Ромашковского")).toHaveLength(1);
  });
});

describe("privacy/deid/precision: округ сокращением вперёд", () => {
  const per = (raw: string, index: number): DetectedEntity => ({
    type: "PER", raw, index, confidence: "high", source: "rule",
  });
  const one = (text: string, raw: string): DetectedEntity[] =>
  ...
  it("«Г.О. Ромашкинский» рядом с адресом — округ, а не человек", () => {
    const text = "|Место нахождения: 309508, Белгородская область, Г.О. Ромашкинский, г. Ромашкин, ул. Мира|";
    expect(one(text, "Г.О. Ромашкинский")).toEqual([]);
  });

  it("живой «Г.О. Ромашкин» с фамилией остаётся скрытым", () => {
    // За инициалами стоит фамилия, а не прилагательное на «-ский».
    const text = "|Место нахождения: 309508, Белгородская область, г. Ромашкин, ул. Мира| Г.О. Ромашкин";
    expect(one(text, "Г.О. Ромашкин")).toHaveLength(1);
  });

  it("без адреса рядом правило молчит", () => {
    const text = "Акт подписал Г.О. Ромашкинский лично, замечаний нет";
    expect(one(text, "Г.О. Ромашкинский")).toHaveLength(1);
  });
});

describe("privacy/deid/precision: согласованность топонима по документу", () => {
  const per = (raw: string, index: number): DetectedEntity => ({
    type: "PER", raw, index, confidence: "high", source: "rule",
  });

  it("то же имя без типа улицы рядом тоже не скрывается", () => {
    const text = "Г САРАТОВ, ЛН. 13-Я (2-Й ИМ | |РОМАШКИНА Е.И. МКР.), Д. Е115 | ... РОМАШКИНА Е.И., ЛИНИЯ 13-Я, Д. Е115";
    const first = text.indexOf("РОМАШКИНА Е.И.");
    const second = text.lastIndexOf("РОМАШКИНА Е.И.");
    expect(refinePersons(text, [per("РОМАШКИНА Е.И.", first), per("РОМАШКИНА Е.И.", second)])).toEqual([]);
  });

  it("подписант с той же фамилией остаётся скрытым", () => {
    // Подписная подсказка отменяет ПЕРЕНОС решения на другие вхождения, а не само решение:
    // адрес остаётся адресом, но однофамилец в подписи прячется. Так же устроено и
    // мемориальное правило.
    const text = "Г САРАТОВ, ЛН. 13-Я (2-Й ИМ | |РОМАШКИНА Е.И. МКР.), Д. Е115 ... в лице директора РОМАШКИНА Е.И., действующего";
    const first = text.indexOf("РОМАШКИНА Е.И.");
    const second = text.lastIndexOf("РОМАШКИНА Е.И.");
    const kept = refinePersons(text, [per("РОМАШКИНА Е.И.", first), per("РОМАШКИНА Е.И.", second)]);
    expect(kept.map((e) => e.index)).toEqual([second]);
  });
});
