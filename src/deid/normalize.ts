...
function prepare(source: string): Prepared {
  ...
  for (let at = 0; at < src.length; at++) {
    ...
    const emphasis = emphasisRunLength(src, at);
    if (emphasis > 0) {
      changed = true;
      at += emphasis - 1;
      continue;
...
/**
 * Разметка выделения markdown: `**`, `*`, `__` вокруг слова.
 *
 * ЗАЧЕМ. Наш же продовый путь распознавания скана (Gemini) отдаёт текст в markdown, и ячейки
 * таблиц приезжают как `**Холой**`. Для правил это НЕ буква: «в системе реки **Холой**» не
 * подходит ни под одно правило вида «ключ + Заглавная», потому что сразу за пробелом стоит
 * звёздочка. Замер 06.08 на буровых журналах клиента: так терялись названия рек и ручьёв в
 * шапках журналов и номера скважин («от скважины той же линии № **4**»), а это самый частый
 * жанр у недропользователя. Симптом выглядел как «детектор не знает форму», хотя форму он
 * знает — не знает разметки.
 *
 * ПОЧЕМУ ЗДЕСЬ, А НЕ В ПРАВИЛАХ. Иначе звёздочку пришлось бы вписать в каждое правило и в
 * каждое новое, а тут она снимается один раз для всего стека, и офсеты возвращаются в
 * исходный текст штатной картой позиций — в сейф и в документ по-прежнему попадает то, что
 * реально написано.
 *
 * ГРАНИЦА. Снимаем только там, где знак ведёт себя как разметка: примыкает к тексту РОВНО с
 * одной стороны. «5*3» и «два_три» не трогаем (текст с обеих сторон), «№ ___» и «5 * 3» тоже
 * (с обеих сторон пробел). Считаем именно НЕ-ПРОБЕЛ, а не букву: закрывающая пара часто
 * стоит за знаком препинания — «**Обозначения скважин:**», — и по букве такой хвост не
 * снимался, из-за чего список номеров скважин за ним оставался невидимым для правил.
 * Fail-open, как и вся нормализация: сомнительное оставляем как есть.
 */
const EMPHASIS = new Set(["*", "_"]);
function emphasisRunLength(src: string[], at: number): number {
  const ch = src[at]!;
  if (!EMPHASIS.has(ch)) return 0;
  // Середина уже осмотренного пробега: решение принимается ОДИН раз, на его первом знаке.
  // Иначе у «№ ___» последний знак виден как «слева текст, справа пробел» и снимается
  // поодиночке, хотя весь пробег — это пустая графа бланка, а не разметка.
  if (at > 0 && src[at - 1] === ch) return 0;
  let end = at;
  while (end + 1 < src.length && src[end + 1] === ch) end++;
  const len = end - at + 1;
  // Подчёркивание разметкой считаем только парное: одиночное — это заполняемый бланк.
  if (ch === "_" && len < 2) return 0;
  // Не длиннее двух: жирный — это «**», курсив «*», а «***» в судебных решениях — МАСКА
  // уже обезличенного значения («по договору № ***от 20.07.2020»). Снимать её нельзя:
  // текст под ней склеивается, и правила начинают видеть номер там, где его вырезали.
  if (len > 2) return 0;
  const before = at > 0 ? src[at - 1]! : "";
  const after = end + 1 < src.length ? src[end + 1]! : "";
  const tightBefore = before !== "" && !/\s/u.test(before);
  const tightAfter = after !== "" && !/\s/u.test(after);
  return tightBefore !== tightAfter ? len : 0;
}
...
export function softenAllCaps(source: string): string {
  return source.replace(ALL_CAPS_PHRASE, (phrase) =>
    phrase.replace(CAPS_WORD, (word) => word.charAt(0) + word.slice(1).toLowerCase()),
  );
}

/**
 * Капсовая ФРАЗА, а не отдельное слово.
 *
 * Порог в пять букв ставился против аббревиатур (ООО, ИНН, НИИ), и одиночное слово он
 * разбирает верно. Но в капсовом ЗАГОЛОВКЕ короткие слова стоят вперемешку с длинными, и
 * порог рвал фразу посередине: «В ПРЕДЕЛАХ МУНГИНСКОГО РУДНОГО УЗЛА» смягчалось до
 * «Пределах Мунгинского Рудного УЗЛА», потому что в «УЗЛА» ровно четыре буквы, — а гео-правилу
 * нужен ключ «рудный узел» целиком, и название рудного узла оставалось открытым (замер 06.08
 * на архиве недропользователя).
 *
 * Признак, отделяющий короткое слово заголовка от аббревиатуры, — СОСЕДСТВО: аббревиатура
 * стоит среди обычного текста, а слово заголовка — рядом с другим капсовым словом. Поэтому
 * фраза обязана НАЧИНАТЬСЯ с длинного капсового слова, а продолжаться может любыми капсовыми.
 * Одинокое «ООО» такой фразы не образует и остаётся нетронутым.
 *
 * Длина не меняется ни на знак: смягчается каждое слово по отдельности, разделители целы.
 */
const CAPS_WORD = /[А-ЯЁ]+(?:[-‑][А-ЯЁ]+)*/gu;
const ALL_CAPS_PHRASE =
  /[А-ЯЁ]{5,}(?:[-‑][А-ЯЁ]{2,})*(?:[  ]+[А-ЯЁ]{2,}(?:[-‑][А-ЯЁ]{2,})*)*/gu;
