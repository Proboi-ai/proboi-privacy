 ...
 * а это ХУЖЕ лишних номеров. Поэтому падеж выбирается по месту, четырьмя ступенями:
 *
 *   0. ДОСЛОВНО ПО ВХОЖДЕНИЮ. Возврат обслуживает ДВА разных случая, и до 30.07 они шли одним
 *      кодом: «вернуть тот же документ» (обязан быть побайтово точным) и «вернуть текст,
 *      который написала модель» (там нужна подстановка по падежу подсказки). Из-за общего
 *      пути 40,2% договоров возвращались с другим окончанием, чем было в оригинале
 *      (независимый замер на договорах ЕИС 29.07). Теперь при обезличивании запоминается
 *      КАЖДОЕ вхождение по порядку — слово слева и точное написание, — и если k-й ярлык
 *      стоит после того же слова, что и k-е вхождение, подставляется ровно оно. Документ
 *      не тронут → рейс дословный; модель переписала окружение → слово слева не совпало,
 *      и работают ступени ниже, как раньше.
...
export interface IdentityVault {
  ...
  occurrenceAt?(token: string, nth: number): { cue: string; form: string } | undefined;
 ...
 *
 * `nth` — какой по счёту это ярлык ЭТОГО токена в тексте (с нуля). Знает его только тот, кто
 * обходит текст, поэтому считает вызывающий. Не передан → ступень 0 не работает и поведение
 * ровно прежнее.
 */
export function originalFor(
  ...
  // Ступень 0 — дословно. Для ВСЕХ типов, а не только для ФИО: тот же документ обязан
  // вернуться побайтово, а канон у любого типа — форма ПЕРВОГО вхождения, и на втором
  // вхождении она уже могла отличаться (капс в подписи, перенос строки внутри названия).
  if (nth !== undefined) {
    const at = vault.occurrenceAt?.(token, nth);
    if (at !== undefined && at.cue === cue) return at.form;
  }
