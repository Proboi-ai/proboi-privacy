...
function nameWordTouches(text: string, index: number, length: number): boolean {
  ...
  if (before && !ROLE_WORDS.has(before[1]!.toLowerCase())) return true;
...
const RULES: Rule[] = [
  ...
  // PER: Фамилия Имя Отчество — во всех падежах («Ивановым Иваном Петровичем»).
  {
    type: "PER",
    re: new RegExp(
      `(?<![А-Яа-яЁё-])${SURNAME}\\s+${CAPITALIZED}\\s+${PATRONYMIC}(?![а-яё])`,
      "gu",
    ),
    confidence: "medium",
  ...
  // PER: Имя Отчество Фамилия — прямой порядок, тоже во всех падежах.
  {
    type: "PER",
    re: new RegExp(
      `(?<![А-Яа-яЁё-])${CAPITALIZED}\\s+${PATRONYMIC}\\s+${SURNAME}(?![а-яё])`,
      "gu",
    ),
    confidence: "medium",
  },
  ...
  {
    type: "PER",
    re: new RegExp(`(?<![А-Яа-яЁё-])${CAPITALIZED}\\s+${PATRONYMIC}(?![а-яё])`, "gu"),
    confidence: "medium",
